@extends('front_end.layout.master')
@section('title','Hoạt động')
@section('content')
    <div class="top-event">
        <div class="container">
            <div class="_lstev">
                <div class="_tith1">
                    <h1><a href="#">sự kiện tiêu biểu</a></h1>
                </div>
                <div class="lstev">
                    <div class="dsevt">
                        <div class="_listsk">
                            <ul>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                                <li>
                                    <div class="lstevimg">
                                        <img src="{{asset('images/sk6.jpg')}}" alt="">
                                    </div>
                                    <div class="lstevct">
                                        <h3><a href="#">Thái Bình trọn niềm vui</a></h3>
                                        <p>Nhiều nghệ sĩ nổi tiếng của Việt Nam đã về Thái Bình
                                            tham gia chương trình nghệ thuật đón năm mới Canh Tý 2020</p>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div class="clcknxt">
                            <button>1</button>
                            <button>2</button>
                            <button>3</button>
                            <button><i class="fa fa-angle-right" aria-hidden="true"></i></button>
                            <button><i class="fa fa-angle-double-right" aria-hidden="true"></i></button>
                        </div>
                    </div>
                    <div class="hgtcl">
                        <div class="_gtcl">
                            <h3>hệ giá trị cốt lõi</h3>
                            <ul>
                                <li><i class="fa fa-check-circle" aria-hidden="true"></i><span>chủ động:</span><p>Luôn chủ động trong mọi vấn đề, tình huống.</p></li>
                                <li><i class="fa fa-check-circle" aria-hidden="true"></i><span>trách nhiệm:</span><p>Thể hiện tinh thần trách nhiệm mọi lúc, mọi nơi.</p></li>
                                <li><i class="fa fa-check-circle" aria-hidden="true"></i><span>sáng tạo:</span><p>Không ngại sáng tạo, bứt phá thành công.</p></li>
                                <li><i class="fa fa-check-circle" aria-hidden="true"></i><span>tôn trọng:</span><p>Tinh thần tôn trọng đối tác, khách hàng.</p></li>
                                <li><i class="fa fa-check-circle" aria-hidden="true"></i><span>đạo đức:</span><p>Đạo đức luôn là nền tảng của sự phát triển.</p></li>
                            </ul>
                            <p>Hệ giá trị cốt lõi được hình thành dựa trên sự đúc kết của toàn bộ tập thể ekip Phi Long Multimedia,. JSC qua nhiều năm thực hiện các chương trình
                                với nhiều đối tác trong nước & quốc tế
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    @endsection