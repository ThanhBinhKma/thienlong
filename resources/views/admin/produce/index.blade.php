                ...
                <div class="box">
                    <div class="box-header with-border">
                        <div class="pull-right">
                            ...
                            <div class="btn-group pull-right" style="margin-right: 10px">
                                <a href="{{route('system_admin.produce.create')}}" class="btn btn-sm btn-success"
                                   title="New">
                ...
                <div class="portlet-title">
                  ...
                  <button class="btn btn-primary btn-show-table-options">
                                ...
                                @foreach($produces as $produce)
                                    <tr>
                                        <td><input type="checkbox" class="grid-row-checkbox"
                                                   data-id="{{ $produce->id }}"/></td>
                                        <td>
                                            {{ $produce->id }}
                                        </td>
                                        <td>
                                            <a class="text-left" href=""
                                               title="{{ $produce->title }}">{{ $produce->title }}</a>

                                        </td>
                                        <td style="width: 100px;">
                                            <img src="{{$produce->avatar}}" alt="" style="max-width: 100%">
                                        </td>
                                        <td>
                                            {{$produce->date}}
                                        </td>
                                        ...
                                        <td> {{ \Carbon\Carbon::parse($produce->created_at)->format('d/m/Y H:i:s')}}</td>
                                        <td> {!! \App\Helpers\Common::checkStatus($produce->status) !!} </td>
                                        <td>
                                            <a href="{{route('system_admin.produce.edit',['id'=>$produce->id])}}"
                                            ...
                                            <a href="javascript:void(0);" data-id="{{$produce->id}}"
                    ...
                    <div class="box-footer clearfix">
                        <div class="col-md-5">
                            Hiển thị trang <b>{{ $produces->currentPage() }}</b> / <b>{{ $produces->lastPage() }}</b>
                        </div>
                        <div class="col-md-7">
                            {{
                              $produces->appends([
