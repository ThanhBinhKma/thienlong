        ...
        <div class="box">
          <div class="box-header with-border">
            <div class="pull-right">
              ...
              <div class="btn-group pull-right" style="margin-right: 10px">
                <a href="{{route('system_admin.account.create')}}" class="btn btn-sm btn-success" title="New">
            ...
            <table class="table table-hover">
              ...
              <tbody>
                  ...
                  @foreach($account as $accounts)
                    <tr>
                      <td><input type="checkbox" class="grid-row-checkbox" data-id="{{ $accounts->id }}" /></td>
                      <td>
                        {{ $accounts->id }}
                      </td>
                      <td>
                        <a class="text-left" href="{{route('system_admin.account.edit',['id'=>$accounts['id']])}}" title="{{ $accounts['account_name'] }}">{{ $accounts['account_name'] }}</a>
                      </td>
                      <td> {!! \App\Helpers\Common::checkSocial($accounts['social']) !!}</td>
                      <td> {!! \App\Helpers\Common::checkStatus($accounts['status']) !!} </td>
                      <td>
                        <a href="{{route('system_admin.account.edit',['id'=>$accounts['id']])}}" class="btn btn-icon btn-sm btn-primary tip">
                          <i class="fa fa-eye"></i>
                        </a>
                        <a href="javascript:void(0);" data-id="{{ $accounts->id }}" class="btn btn-icon btn-sm btn-danger deleteDialog tip">
          ...
          <div class="box-footer clearfix">
            <div class="col-md-5">
               Hiển thị trang <b>{{ $account->currentPage() }}</b> / <b>{{ $account->lastPage() }}</b>
            </div>
            <div class="col-md-7"> 
              {{ 
                $account->appends([
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      $('.deleteDialog').on('click', function() {
        ...
        destroy( page_id , '{{ route('system_admin.account.destroy') }}' , '{{ route('system_admin.account.index') }}' , "Bạn muốn xóa trang này!" );
        console.log(page_id);

      });
    });
    $('.grid-batch-0').on('click', function() {
      destroyAll( '{{ route('system_admin.account.destroyAll') }}' , '{{ route('system_admin.account.index') }}' , "Bạn muốn xóa các trang đã chọn?" );
    });
    $('.grid-batch-1').on('click', function() {
      restore( '{{ route('system_admin.account.restore') }}' , '{{ route('system_admin.account.index') }}' , "Bạn muốn phục hồi các trang đã chọn?" );
    });
  </script>
@stop